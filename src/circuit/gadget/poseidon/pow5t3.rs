...
impl<F: FieldExt> Pow5T3Chip<F> {
    ...
    pub fn configure<S: Spec<F, WIDTH, 2>>(
        ...
        let pad_and_add = |meta: &mut ConstraintSystem<F>, idx: usize| {
            ...
            s_pad_and_add * (initial_state + input - output_state)
        };
        meta.create_gate("pad-and-add state_0", |meta| pad_and_add(meta, 0));
        meta.create_gate("pad-and-add state_1", |meta| pad_and_add(meta, 1));
        meta.create_gate("pad-and-add state_2", |meta| {
            let initial_state = meta.query_advice(state[2], Rotation::prev());
            let output_state = meta.query_advice(state[2], Rotation::next());
            let s_pad_and_add = meta.query_selector(s_pad_and_add, Rotation::cur());

            // The capacity element is never altered by the input.
            s_pad_and_add * (initial_state - output_state)
        });
...
impl<F: FieldExt, S: Spec<F, WIDTH, 2>> PoseidonInstructions<S, WIDTH, 2> for Pow5T3Chip<F> {
...
impl<F: FieldExt, S: Spec<F, WIDTH, 2>> PoseidonDuplexInstructions<S, WIDTH, 2> for Pow5T3Chip<F> {
    fn initial_state(
        layouter: &mut impl Layouter<Self>,
        domain: &impl Domain<Self::Field, S, WIDTH, 2>,
    ) -> Result<State<Self::Word, WIDTH>, Error> {
        let config = layouter.config().clone();
        layouter.assign_region(
...
mod tests {
    ...
    impl Circuit<Fp> for PermuteCircuit {
        ...
        fn synthesize(
            ...
            let final_state = <Pow5T3Chip<_> as PoseidonInstructions<
                ...
                WIDTH,
                2,
            >>::permute(&mut layouter, &initial_state)?;
    ...
    impl Circuit<Fp> for HashCircuit {
        ...
        fn synthesize(
            ...
            let mut layouter =
                layouter::SingleChip::<'_, Pow5T3Chip<Fp>, _>::new(cs, config.clone())?;
            ...
            let message = layouter.assign_region(
                ...
                |mut region| {
                    let mut message_word = |i: usize| {
                        ...
                        Ok(Word::<_, OrchardNullifier, WIDTH, 2> {
            ...
            let hasher = Hash::init(layouter.namespace(|| "init"), ConstantLength::<2>)?;
